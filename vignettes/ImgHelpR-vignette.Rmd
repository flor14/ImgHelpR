...
-   `Crop()` cuts an image's dimensions to a desired size. There are two inputs, `height` and `width` that must be inputted by the user. The function returns an array, which can be read into `rasterImage` as shown in the **Data** section.
